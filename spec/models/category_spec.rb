...
describe Category do
  it "has a valid fabrication" do
    Fabricate(:category).should be_valid
  end

  it "is invalid without a title" do
    Fabricate.build(:category, title: nil).should_not be_valid
  end

  it "creates a parameterized name from title" do
    record = Fabricate.build(:category, title: "Bedroom Sets")
    record.save
    record.name.should == "bedroom-sets"
  end

  it "has a link parameter" do
    Fabricate.build(:category, link: nil).should be_valid
  end

  it 'accepts blank for link' do
    Fabricate.build(:category, link: "").should be_valid
  end

  it 'only accepts "http://", "https://", and "/" prefixed to links when filled in' do
    # valid
    Fabricate.build(:category, link: "http://").should be_valid
    Fabricate.build(:category, link: "https://").should be_valid
    Fabricate.build(:category, link: "/").should be_valid
    
    # invalid
    Fabricate.build(:category, link: "locations/10").should_not be_valid
    Fabricate.build(:category, link: "lskdfjldskf").should_not be_valid
  end
end
